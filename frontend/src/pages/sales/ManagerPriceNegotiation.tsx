...
import { getQuotations, managerReview } from '../../services/quotationService.js';
...
export default function ManagerPriceNegotiation() {
  ...
  const fetchQuotations = async () => {
    try {
      ...
      // Lọc ra những báo giá đang ở trạng thái PendingManager nếu Backend chưa lọc
      const pendingManagerData = Array.isArray(data) 
        ? data.filter(q => q.status === 'PendingManager') 
        : [];
      setRows(pendingManagerData);
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  const executeReview = async () => {
    ...
    try {
      await managerReview(id, { isApproved: isApprove, managerNote: note });
        ...
        <table className="w-full">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={7} className="text-center py-4 text-sm text-gray-500">Không có báo giá nào chờ Quản lý duyệt</td>
              </tr>
            ) : (
              rows.map((row) => {
